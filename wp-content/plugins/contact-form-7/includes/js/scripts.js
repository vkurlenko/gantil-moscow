(function($) {
	...
	$('.speaker-topic-title > h4 > a').click(function()
	{
		/*
		...
		*/
		...
		var master_name = $(this).parents('.our-team').find('h5').text();
		var str = $(this).parents('.our-team').find('p').eq(1).html(); 
		var master_info = str.split("<br>");
		var master_salon = $.trim(master_info[1])
		
		if(master_salon == 'салон на Ленинском') var email = mail[0];
		if(master_salon == 'салон на Коломенской') var email = mail[1];
		if(master_salon == 'салон на Братиславской') var email = mail[2];
		if(master_salon == 'салон на Щелковской') var email = mail[3];
		if(master_salon == 'салон на Соколе') var email = mail[4];
		if(master_salon == 'салон на Сходненской') var email = mail[5];
		if(master_salon == 'Дом Красоты м. Аэропорт') var email = mail[6];
		
		
		
		//alert(master_salon)
		var master_photo = $(this).parents('.our-team').find('.team-member').html();
		...
		$('#master_info').remove()
		$('.wpcf7-form').find('p').eq(2).prepend('<p id="master_info" style="text-align:center"></p>');
